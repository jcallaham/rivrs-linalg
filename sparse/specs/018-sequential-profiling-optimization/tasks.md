...
- [X] T026 [US4] Write `dev/phase-8.1g-report.md` summarizing the performance analysis: (1) Front size distribution across the SuiteSparse suite, (2) Per-supernode time contribution analysis (what fraction of total time is in the top 10% of fronts), (3) Allocation pressure before/after optimization (from US2 comparison), (4) Matrix classification table (TreeLevel/IntraNode/Mixed), (5) Parallelism strategy recommendation for Phase 8.2 with evidence from at least three representative matrix classes, (6) Chrome Trace examples for representative matrices.
...
- [X] T030 Update `dev/ssids-log.md` with Phase 8.1g development log entry: what was built, what was changed, what was found, and what it means for Phase 8.2.
- [X] T031 Update `dev/ssids-plan.md` to mark Phase 8.1g as COMPLETE and record key findings (parallelism recommendation, allocation reduction results).
